...
extern void SearchFreqModeDisplayDCSData(U8 ctsDcsType, U32 dat, U8 isStandard)
{
    String disBuf[12] = {0};
    memset(disBuf, ' ', 11);
    LCD_DisplayText(49, 37, (U8 *)&disBuf, FONTSIZE_12, LCD_DIS_NORMAL);
...
extern void SearchFreqModeDisplayStepMsg(ENUM_SEARCHFREQ_STEP step)
{
    ...
    {
        LCD_DisplayText(17, 108, "UHF", FONTSIZE_12, LCD_DIS_NORMAL);
    }
    else if (searchFreqImofs.band == FREQ_BAND_200M)
    {
        LCD_DisplayText(17, 108, "200", FONTSIZE_12, LCD_DIS_NORMAL);
    }

    else if (searchFreqImofs.band == FREQ_BAND_350M)
    {
        LCD_DisplayText(17, 108, "350", FONTSIZE_12, LCD_DIS_NORMAL);
    }
    else
    {
        LCD_DisplayText(17, 108, "VHF", FONTSIZE_12, LCD_DIS_NORMAL);
    }

    if (step == STEP_SEEK_FREQ)
    {
        LCD_DisplayText(33, 37, (U8 *)&disBuf, FONTSIZE_12, LCD_DIS_NORMAL);
...
extern void SearchFreqDisplayHome(void)
{
    ...
    // 显示扫频标志
    LCD_DisplayText(17, posx, (U8 *)((g_radioInform.language == LANG_CN) ? "扫频" : "SEARCH"), FONTSIZE_12, LCD_DIS_NORMAL);
    // 显示频率
