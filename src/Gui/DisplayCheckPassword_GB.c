...
{
    LCD_ClearFullBuf();
    LCD_DisplayText(20, 8, (U8 *)((g_radioInform.language == LANG_CN) ? "请输入开机密码" : "Input Password"), FONTSIZE_16, LCD_DIS_NORMAL);
    DisplayPassword();
...
{
    LCD_ClearFullBuf();
    LCD_DisplayText(23, 12, (U8 *)((g_radioInform.language == LANG_CN) ? "   密码错误   " : "Password Error"), FONTSIZE_16, LCD_DIS_NORMAL);
    LCD_UpdateFullScreen();
    DelayMs(2000);
    ResetInputBuf();
    DisplayPasswordHome();
